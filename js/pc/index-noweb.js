		...
		for (var i = 0 , k = 0 ; i < _count ; i ++ ) {
			...
			for ( var j = 0 ; j < _amount ; j ++ , k ++ ) {
				if ( k >= 0 && k < Datas.Data[_type].length ) {
					...
					_Str.push('	<span class="pic middleSet"><img src="http://brianlin224.theweb.tw/img/pc/' + Datas.Data[_type][k].CaseType + '/' + Datas.Data[_type][k].CoverImg + '" alt="' + Datas.Data[_type][k].CaseName + '"></span>');
		...
		for (var i = 0; i < Datas.Data[_ls_return].length; i++) {
			if (Datas.Data[_ls_return][i].CaseType + '_' + Datas.Data[_ls_return][i].CaseID == _id) {
				for (var j = 1; j <= parseInt(Datas.Data[_ls_return][i].PhotoCount); j++) {
					...
					_list.push('<li class="list middleSet"><img src="http://brianlin224.theweb.tw/img/pc/' + Datas.Data[_ls_return][i].CaseType + '/detail/' + Datas.Data[_ls_return][i].CaseType + Datas.Data[_ls_return][i].CaseID + '_0' + j + '.png?v=20150214"></li>');
				}
