$(function(){
	var $screen    = $('.screen'),
		...
		_mainView  = ['<div class="mainWrap front-end">',
						'<div class="logo">',
							'<h1 class="logo-img"><img src="img/mobile/signature.png" alt="Brian Lin signature"></h1>',
						'</div>',
						'<h2 class="title">DESIGN &amp; CREATIVE PORTFOLIO</h2>',
						'<ul class="works"></ul>',
					'</div>'],
		_aboutView = ['<div class="mainWrap about">',
						'<span class="head-shot"><img src="img/mobile/head-shot.png" alt="Brian Lin`s photo"></span>',
	...
	// 切換至PC版
	if ($(window).width() > 860) {
		window.location.replace(_url + 'index-local.html');
	} else {
		appendItem();
	}
	...
	$(window).resize(function(){
		if ($(window).width() > 860) {
			window.location.href = _url + 'index-local.html';
		}
	});
